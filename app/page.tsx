        ...
        {esMiembro ? (
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
            ...
            {imagenPreview ? (imagenPreview.includes('.gif') ? (
              ...
              <div className="mt-3 relative"><img src={imagenPreview} className="w-full max-h-80 object-cover rounded-lg" alt="Preview" /><button onClick={limpiarMultimedia} className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1 hover:bg-black/70 cursor-pointer"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button></div>
            )) : null}

            {(nuevoPost.trim() !== "" || imagenPreview) ? (
              <div className="mt-3 flex justify-between items-center border-t border-gray-200 pt-3">
                <span className="text-xs text-gray-400">{nuevoPost.length}/1000</span>
                <button className="bg-[#1877F2] hover:bg-[#166FE5] text-white font-bold py-2 px-6 rounded-md text-sm cursor-pointer" onClick={publicarPost}>Publicar</button>
              </div>
            ) : null}
          </div>
        ...
        <div className="flex flex-col gap-4">
          {posts.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center text-gray-500">
              <p className="text-lg font-medium mb-2">No hay publicaciones todavia</p>
              <p className="text-sm">Se el primero en compartir algo en este grupo.</p>
            </div>
          ) : (
            posts.map((post) => (
              <div key={post.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                ...
                <div className="border-t border-gray-200 mt-2 pt-2 flex justify-between text-gray-500 text-sm">
                  <span onClick={() => toggleLikePost(post.id)} className={`px-4 py-1 rounded cursor-pointer flex items-center gap-1 transition-colors ${likedPosts.has(post.id) ? 'bg-blue-50 text-blue-600 font-bold' : 'hover:bg-gray-100'}`}>
                    {likedPosts.has(post.id) ? '👍 Liked' : '👍 Like'} ({post.likes || 0})
                  </span>
                  <span onClick={() => toggleComentarios(post.id)} className={`px-4 py-1 rounded cursor-pointer ${postsAbiertos.includes(post.id) ? "font-bold text-gray-900 bg-gray-100" : "hover:bg-gray-100"}`}>
                  ...
                  {(post.user_id === usuario?.id || miRol === 'admin' || miRol === 'moderador') ? (
                    <span onClick={() => eliminarPost(post.id)} className="hover:bg-red-100 hover:text-red-600 text-gray-400 px-4 py-1 rounded cursor-pointer">Eliminar</span>
                  ) : null}
                </div>
                ...
                {postsAbiertos.includes(post.id) && (
                  <div className="mt-3 pt-3 border-t border-gray-100">
                    ...
                    {/* COMENTARIOS SEPARADOS POR LÍNEAS PARA EVITAR ERRORES */}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between mb-0.5">
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  ...
                                  <span className={`text-[11px] font-bold ${c.userVote === 1 ? 'text-orange-500' : c.userVote === -1 ? 'text-blue-500' : 'text-gray-500'}`}>
                                    {c.score || 0}
                                  </span>
